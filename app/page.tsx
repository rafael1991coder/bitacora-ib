...
import { BookOpen, Check, ExternalLink, FileText, Image as ImageIcon, KeyRound, LayoutGrid, List, LogOut, MessageSquare, Plus, Search, ShieldCheck, Star, Trash2, UserRound, Video, X } from 'lucide-react';
...
type Feedback = { resourceId: string; userId: string; userName: string; rating: number; comment: string; createdAt: string; updatedAt: string };
type Resource = { id: string; title: string; url: string; type: ResourceType; ownerId: string; studiedBy: string[]; feedback: Feedback[]; createdAt: string };
...
export default function Home() {
  ...
  async function saveFeedback(resourceId: string, rating: number, comment: string) { if (!currentUserId) throw new Error('Ingresa con tu usuario para calificar.'); await apiAction({ action: 'saveFeedback', userId: currentUserId, password: sessionPassword, resourceId, rating: String(rating), comment }); await refreshData(); flash('Tu calificación quedó compartida con el equipo.'); }
  ...
  return <main className="min-h-screen bg-[#f3f7f6] text-[#122b2a]">
    ...
    <section className="mx-auto max-w-7xl px-5 py-8 lg:px-8 lg:py-10">
      ...
      <div className="mb-6 flex flex-col gap-3 rounded-2xl border border-[#d9e5e2] bg-white p-3 shadow-[0_8px_30px_rgba(31,78,74,.05)] sm:flex-row sm:items-center"><label className="flex flex-1 items-center gap-2 rounded-xl bg-[#f2f7f6] px-3"><Search className="size-4 text-[#6f8986]" /><Input aria-label="Buscar recursos" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por título, vínculo o persona…" className="h-11 border-0 bg-transparent px-0 shadow-none focus-visible:ring-0" /></label><NativeSelect aria-label="Filtrar por tipo" value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)} className="w-full sm:w-44"><NativeSelectOption value="todos">Todos los tipos</NativeSelectOption><NativeSelectOption value="video">Videos</NativeSelectOption><NativeSelectOption value="documento">Documentos</NativeSelectOption><NativeSelectOption value="imagen">Imágenes</NativeSelectOption></NativeSelect><div className="flex rounded-lg bg-[#f2f7f6] p-1" aria-label="Modo de vista"><button type="button" onClick={() => setViewMode('mosaico')} aria-label="Vista en mosaico" className={`grid size-9 place-items-center rounded-md ${viewMode === 'mosaico' ? 'bg-white text-[#0e615d] shadow-sm' : 'text-[#6f8986]'}`}><LayoutGrid className="size-4" /></button><button type="button" onClick={() => setViewMode('lista')} aria-label="Vista en lista" className={`grid size-9 place-items-center rounded-md ${viewMode === 'lista' ? 'bg-white text-[#0e615d] shadow-sm' : 'text-[#6f8986]'}`}><List className="size-4" /></button></div></div>
      {filteredResources.length === 0 ? <div className="rounded-3xl border border-dashed border-[#b9cfcc] bg-white/60 px-6 py-16 text-center"><div className="mx-auto mb-4 grid size-14 place-items-center rounded-2xl bg-[#dceeea] text-[#0e615d]"><BookOpen /></div><h2 className="font-serif text-2xl font-bold">La biblioteca está lista</h2><p className="mx-auto mt-2 max-w-md text-[#5d7774]">Registra el primer video, documento o imagen que estés estudiando del IB.</p></div> : <div className={viewMode === 'mosaico' ? 'grid gap-4 md:grid-cols-2 xl:grid-cols-3' : 'grid gap-3'}>{filteredResources.map((resource) => { const info = typeInfo[resource.type]; const Icon = info.icon; const owner = data.users.find((user) => user.id === resource.ownerId); const studied = !!currentUserId && resource.studiedBy.includes(currentUserId); const studiedNames = resource.studiedBy.map((id) => data.users.find((user) => user.id === id)?.name).filter(Boolean); return <article key={resource.id} className={`group flex flex-col rounded-2xl border border-[#d9e5e2] bg-white p-5 shadow-[0_8px_28px_rgba(31,78,74,.05)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(31,78,74,.10)] ${viewMode === 'mosaico' ? 'min-h-64' : 'md:grid md:grid-cols-[auto_minmax(0,1fr)_minmax(240px,.7fr)] md:items-start md:gap-5'}`}><div className="mb-5 flex items-start justify-between gap-3 md:mb-0"><span className={`inline-flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs font-bold uppercase tracking-wider ${info.color}`}><Icon className="size-4" />{info.label}</span>{resource.ownerId === currentUserId && <Button onClick={() => deleteResource(resource.id)} variant="ghost" size="icon-sm" aria-label="Eliminar recurso" className="text-[#78908e] hover:text-red-700"><Trash2 /></Button>}</div><div><h2 className="font-serif text-xl font-bold leading-snug text-[#173f3c]">{resource.title}</h2><p className="mt-2 line-clamp-1 text-sm text-[#6b817f]">Compartido por {owner?.name ?? 'Usuario'}</p><div className="mt-4 md:hidden"><FeedbackPanel feedback={resource.feedback ?? []} currentUserId={currentUserId} onSave={(rating, comment) => saveFeedback(resource.id, rating, comment)} /></div></div><div className="mt-auto pt-6 md:mt-0 md:pt-0"><a href={resource.url} target="_blank" rel="noreferrer" className="mb-4 flex items-center justify-between rounded-xl bg-[#f2f7f6] px-3 py-3 text-sm font-bold text-[#0e615d] transition hover:bg-[#e4f1ee]">Abrir recurso <ExternalLink className="size-4" /></a><label className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-3 transition ${studied ? 'border-[#8cc5ba] bg-[#e7f5f1]' : 'border-[#dce7e5] bg-white hover:bg-[#f8fbfa]'}`}><Checkbox checked={studied} onCheckedChange={() => toggleStudied(resource.id)} disabled={!currentUserId} className="size-5" /><span className="flex-1 text-sm font-bold">{studied ? 'Lo estudié' : 'Marcar como estudiado'}</span>{studied && <Check className="size-4 text-[#0e615d]" />}</label>{viewMode === 'mosaico' && <FeedbackPanel feedback={resource.feedback ?? []} currentUserId={currentUserId} onSave={(rating, comment) => saveFeedback(resource.id, rating, comment)} />}{studiedNames.length > 0 && <p className="mt-3 text-xs leading-relaxed text-[#6b817f]"><strong className="text-[#365956]">Estudiado por:</strong> {studiedNames.join(', ')}</p>}</div></article>; })}</div>}
    </section>
...
function FeedbackPanel({ feedback, currentUserId, onSave }: { feedback: Feedback[]; currentUserId: string | null; onSave: (rating: number, comment: string) => Promise<void> }) {
  const mine = feedback.find((item) => item.userId === currentUserId);
  const [rating, setRating] = useState(mine?.rating ?? 0);
  const [comment, setComment] = useState(mine?.comment ?? '');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const average = feedback.length ? feedback.reduce((total, item) => total + item.rating, 0) / feedback.length : 0;

  useEffect(() => { setRating(mine?.rating ?? 0); setComment(mine?.comment ?? ''); }, [mine?.rating, mine?.comment]);

  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!rating) { setError('Selecciona entre una y cinco estrellas.'); return; }
    try { setSaving(true); setError(''); await onSave(rating, comment); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo guardar tu opinión.'); }
    finally { setSaving(false); }
  }

  return <details className="group mt-3 rounded-xl border border-[#dce7e5] bg-[#fbfdfc]">
    <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3 py-2.5 text-sm text-[#365956] [&::-webkit-details-marker]:hidden">
      <span className="flex items-center gap-1.5"><Star className={`size-4 ${feedback.length ? 'fill-[#e3a72f] text-[#e3a72f]' : 'text-[#9aadaa]'}`} /><strong>{feedback.length ? average.toFixed(1) : 'Sin calificar'}</strong><span className="font-medium text-[#728987]">{feedback.length === 1 ? '1 opinión' : `${feedback.length} opiniones`}</span></span>
      <span className="flex items-center gap-1 text-xs font-bold text-[#0e615d]"><MessageSquare className="size-4" />Opinar</span>
    </summary>
    <div className="border-t border-[#e2ece9] p-3">
      {feedback.length > 0 ? <div className="space-y-3">{feedback.map((item) => <div key={item.userId} className="rounded-lg bg-white px-3 py-2.5 text-sm"><div className="flex items-center justify-between gap-2"><strong className="text-[#365956]">{item.userName}</strong><span className="flex text-[#e3a72f]">{[1, 2, 3, 4, 5].map((star) => <Star key={star} className={`size-3.5 ${star <= item.rating ? 'fill-current' : 'text-[#d5dfdd]'}`} />)}</span></div>{item.comment && <p className="mt-1.5 leading-relaxed text-[#627a77]">{item.comment}</p>}</div>)}</div> : <p className="text-sm text-[#728987]">Aún no hay opiniones sobre este recurso.</p>}
      {currentUserId ? <form onSubmit={submit} className="mt-4 border-t border-[#e2ece9] pt-3"><p className="text-xs font-bold uppercase tracking-wide text-[#587370]">{mine ? 'Actualiza tu opinión' : 'Tu opinión'}</p><div className="mt-2 flex gap-1">{[1, 2, 3, 4, 5].map((value) => <button key={value} type="button" onClick={() => setRating(value)} aria-label={`${value} estrellas`} className="rounded p-0.5"><Star className={`size-6 transition ${value <= rating ? 'fill-[#e3a72f] text-[#e3a72f]' : 'text-[#c4d2cf] hover:text-[#e3a72f]'}`} /></button>)}</div><Textarea value={comment} onChange={(event) => setComment(event.target.value)} maxLength={600} placeholder="Añade un comentario opcional..." className="mt-2 min-h-20 resize-y border-[#d5e2df] bg-white text-sm" />{error && <p className="mt-2 text-xs font-medium text-red-700">{error}</p>}<Button type="submit" size="sm" disabled={saving} className="mt-2 bg-[#0e615d] hover:bg-[#084d4a]">{saving ? 'Guardando...' : 'Guardar opinión'}</Button></form> : <p className="mt-3 border-t border-[#e2ece9] pt-3 text-xs text-[#728987]">Ingresa con tu usuario para calificar o comentar.</p>}
    </div>
  </details>;
}
